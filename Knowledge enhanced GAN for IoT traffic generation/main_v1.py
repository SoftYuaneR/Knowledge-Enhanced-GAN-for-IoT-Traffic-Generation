...
from models import BiLSTMGenerator
...
save_dir_head = '/data5/huishuodi/user_gene_results/clu_gen/'
sub_dir = '20220129_clucenterdst_clublc_dscagg_attn/'#'20220123_clucenterdst_clublc_dscagg/'#'20220123_pred_clucenterdst_clublc_dscagg/'
save_dir_log = save_dir_head+'log/'+sub_dir
if not os.path.exists(save_dir_log):
    os.makedirs(save_dir_log)
tb_writer = SummaryWriter()#(save_dir_log)

data_filename = '/data5/huishuodi/pythonProject4trafficGeneration/utf_dataset_file_20220120.npz'
...
for ii in range(NUM_CLUSTERS):
    #netG = MLPGenerator(noise_size=NOISE_SIZE, out_seq_size=LENGTH, hidden_size=hidden_sizes_mlp, condition_size = 32)
    netG = BiLSTMGenerator(input_size=48, noise_size=NOISE_SIZE, out_seq_size=LENGTH, hidden_size=24, condition_size=32, dropout=0.3,
                 num_layers=3)
    net_G_list.append(netG)
netD = TCNDiscriminator(input_size=1, num_clusters=NUM_CLUSTERS, num_channels=None, condition_size=32, condition_squeeze_size=4, dropout=0.3)
#netD_pretrained_file = '/data5/huishuodi/user_gene_results/clu_gen_pretrain/20220121_D_kmeans_6cluster/iteration-1000/netD'
#netD.load_state_dict(torch.load(netD_pretrained_file,map_location=torch.device('cpu')))
#print('pretrained netD loaded!')
...
optimizer_G_list = []
for ii in range(NUM_CLUSTERS):
    optimizer_G = torch.optim.Adam(net_G_list[ii].parameters(), lr=LR, betas=(B1, B2))
...
if True:
    for iteration in trange(ITERS):
        ...
        for idx, (user_id, app_usage, geo_sta, utf_norm, utf, _) in enumerate(data_loader):
            ...
            #clu_criterion = torch.tensor([1/NUM_CLUSTERS]*NUM_CLUSTERS, requires_grad=False).cuda(gpu) if CUDA \
            #    else torch.tensor([1/NUM_CLUSTERS]*NUM_CLUSTERS, requires_grad=False)
            ...
            #real_data = utf.cuda(gpu) if CUDA else utf
            ...
            fake_data = torch.zeros_like(real_data, requires_grad=False)

            D_loss_clu = torch.zeros(1).cuda(gpu) if CUDA else torch.zeros(1)
            for ii in range(NUM_CLUSTERS):
                if (D_real_clusters==ii).any():
                    fake_data[D_real_clusters==ii] = net_G_list[ii](noise[D_real_clusters==ii],
            ...
            #print(D_real[:, 1:].sum(dim=0).shape, clu_criterion.shape)
            #D_loss_clu = cluster_loss(D_real[:, 1:].sum(dim=0)/BATCH_SIZE, clu_criterion)  #MSELoss
            D_loss_clu_blc = - 1/NUM_CLUSTERS * \
                         torch.log(D_real[:, 1:].sum(dim=0)/BATCH_SIZE + 1e-9).sum()  #CrossEntropyLoss
            D_loss = D_loss_adv + D_loss_clu * Bc + D_loss_regularity * Br + D_loss_clu_blc * Bcb
            #print('D:', D_loss)
            D_loss.backward()
            ...
            if idx % CRITIC_ITERS == 0:
                ...
                fake_data = torch.zeros_like(real_data, requires_grad=False) #requires_grad=True?
                for ii in range(NUM_CLUSTERS):
                    if (D_real_clusters == ii).any():
                        net_G_list[ii].zero_grad()
                        fake_data[D_real_clusters == ii] = net_G_list[ii](noise[D_real_clusters == ii],
                                                                      conditions[D_real_clusters == ii])

                D_fake, D_weight = netD(fake_data, conditions)
                #print(fake_data.shape, D_fake.shape, D_fake)
                ...
                for ii in range(NUM_CLUSTERS):
                    ...
                    if idx_clu.any():
                        ...
                        #print(idx_clu.shape, D_fake[idx_clu].shape, D_real[idx_clu].shape, \
                        #      torch.randn(2048).cuda(gpu)[idx_clu].shape, torch.ones_like(idx_clu)[idx_clu].shape)
                        #,gene_criterion, D_fake[D_real_clusters == ii, ii+1].shape)
                        #G_loss_list[ii] = adversarial_loss(D_fake[idx_clu, ii+1], gene_criterion) #MSELoss
                    ...
                    #print('G:', ii, G_loss_list[ii])
                #G_loss_aggregate = aggregate_loss(real_data.sum(0), fake_data.sum(0))
                #G_loss = G_loss + Ba * G_loss_aggregate
                G_loss.backward()
                ...
                #fake_data_np = fake_data_np / fake_data_np.max() if fake_data_np.max() else fake_data_np
                real_data_np = real_data.cpu().detach().numpy()
                #real_data_np = real_data_np / real_data_np.max() if real_data_np.max() else real_data_np
                # print(fake_data_np[0::256].shape, real_data_np[0::256].shape)
